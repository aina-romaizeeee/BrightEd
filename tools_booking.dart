...
class ToolsBookingPage extends StatefulWidget {
  const ToolsBookingPage({super.key});

  @override
  State<ToolsBookingPage> createState() => _ToolsBookingPageState();
}

class _ToolsBookingPageState extends State<ToolsBookingPage>
  ...
  Future<void> _loadData() async {
    ...
    try {
      final data = await ApiService().getTools();
      if (!mounted) return;
      final all = data.cast<Map<String, dynamic>>();
      setState(() {
        _available = all.where((t) {
      ...
      if (!mounted) return;
      setState(() {
        _error = 'Failed to load tools. ${e.toString()}';
        _isLoading = false;
      });
    }
  }

  void _showBookingDialog(Map<String, dynamic> tool) {
                  ...
                  onPressed: () async {
                    ...
                    try {
                      ...
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                              content: Text('Failed to book tool.')),
                        );
                      }
                    }
                  },
  ...
  Future<void> student_cancelBooking(String bookingId) async {
  ...
  Widget _buildToolCard(Map<String, dynamic> tool) {
    ...
    final imageUrl = tool['image_url']?.toString() ?? '';

    return GestureDetector(
      onTap: () => _showBookingDialog(tool),
      child: Container(
        decoration: BoxDecoration(
          color: Colors.white.withOpacity(0.9),
